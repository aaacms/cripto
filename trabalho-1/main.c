...
char* raw_to_hex(const unsigned char *data, size_t len) {
    char *hex_str = malloc((len * 2) + 1);
    if (!hex_str) return NULL;
    
    for (size_t i = 0; i < len; i++) {
        hex_str[i*2]     = hex_digits[data[i] >> 4];
        hex_str[i*2 + 1] = hex_digits[data[i] & 0x0F];
    }
    hex_str[len * 2] = '\0';
    return hex_str;
}
...
char* raw_to_base64(const unsigned char *data, size_t len) {
    ...
    for (i = 0, j = 0; i < len;) {
        ...
        // Divide os 24 bits em 4 grupos de 6 bits e converte para os caracteres correspondentes.
...
unsigned char* base64_to_raw(const char* base64_str, size_t* out_len) {
    ...
    if (len % 4 != 0) return NULL; // O comprimento deve ser múltiplo de 4.
    ...
    for (i = 0, j = 0; i < len;) {
        ...
        if (j < decoded_len) data[j++] = (triple >> 16) & 0xFF;
        if (j < decoded_len) data[j++] = (triple >> 8) & 0xFF;
        if (j < decoded_len) data[j++] = triple & 0xFF;
    }
